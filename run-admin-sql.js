...
const client = new Client({
  host: 'db.vyavdcyidnqedtnvgxlk.supabase.co',
  port: 5432,
  database: 'postgres',
  user: 'postgres',
  password: 'RYANDANNYRYAN98',
...
async function runMigration() {
  try {
    ...
    if (err.message.includes('password authentication failed')) {
      console.error('\n💡 The password might be incorrect. Please check:');
      console.error('   1. Go to https://supabase.com/dashboard/project/vyavdcyidnqedtnvgxlk/settings/database');
      console.error('   2. Click "Reset database password" if needed');
      console.error('   3. Or copy/paste the SQL manually in the SQL editor');
    }
    process.exit(1);
  } finally {
    await client.end();
